...
MANDATORY — BACKGROUND SHAPE, NOT JUST COLOR: if a background or atmospheric element has a distinct GEOMETRIC STRUCTURE — an arc, a ring, a halo, a radiating band, a symmetric curve — describe that SHAPE explicitly in # Composition or # Environment, not just its color or softness. "Blurred multicolored background" loses the structure; "a soft rainbow-hued arc curving symmetrically above the subject" preserves it. A generator that only reads "colorful blur" will not reproduce the shape — replication fidelity depends on naming structure as much as color. Only skip this when the background is genuinely uniform/shapeless — do not invent structure that isn't there.
