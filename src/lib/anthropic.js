...
MANDATORY — GRAIN & NOISE: if the image shows visible noise/grain (film grain, digital sensor noise, or an AI-illustration's uniform textured grain overlay), this is a GOVERNING trait of the medium, not a minor execution note — state its DENSITY (light/moderate/heavy) and CHARACTER (fine vs coarse, uniform vs patchy) explicitly in # Style, in the same sentence that names the medium, not buried later. Heavy uniform grain over soft painterly forms is often the single strongest visual signature of the image — treat it with the same weight as edge character. If the image has no visible grain, state that surfaces are clean/noiseless.
